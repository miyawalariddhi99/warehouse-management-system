...
<html lang="en">
    ...
    <body>
        ...
        <div class="container">
            <h1>Warehouse Data for Surat</h1>
            <?php
            $con = mysqli_connect("localhost:3307", "root", "", "warehouse");
            if (!$con) {
                die("Connection Error !!");
            } else {
                $surat = "SELECT * FROM tblwarehousereg WHERE CityID = 1 AND Verification='APPROVE' AND Availability='AVAILABLE'";
                $q_surat = mysqli_query($con, $surat);
                if ($q_surat) {
                    echo "<table border=1>";
                    echo "<tr>";
                    echo "<th>ID</th>";
                    ...
                    while ($r = mysqli_fetch_assoc($q_surat)) {
